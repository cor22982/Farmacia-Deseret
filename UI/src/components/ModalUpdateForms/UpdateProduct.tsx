...
import { useGetProducts } from 'src/_mock/product';
...
export const UpdateProduct = forwardRef<HTMLDivElement, ModalProductProps>(
  ({ open, close, handleClick, setValueProductId, setCall ,  id_product}, ref) => {
    ...
    const {getOneProductById} = useGetProducts();
    ...
    const {llamadowithFileAndBody, error} = useApi(`${source_link}/updateProduct`)
    ...
    const handleSubmit = async () => {
      ...
      const response = await llamadowithFileAndBody(file, formData, 'PUT');
      if (response) {
